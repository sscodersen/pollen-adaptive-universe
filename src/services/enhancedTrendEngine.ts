    ...
    try {
      ...
      for (const story of hackerNews) {
        if (story.title && story.title.length > 10) {
          ...
          trends.push({
            id: `hn-${story.id || Date.now()}`,
            topic: story.title,
            score: this.calculateTrendScore(sentiment, keywords, 'hackernews'),
            sentiment: sentiment.compound || 0,
            source: 'HackerNews',
            timestamp,
            category: this.categorizeContent(story.title),
            keywords,
            momentum: Math.random() * 100, // Simulate momentum
      ...
      for (const post of redditTech) {
        if (post.title && post.title.length > 10) {
          ...
          trends.push({
            ...
            topic: post.title,
            score: this.calculateTrendScore(sentiment, keywords, 'reddit'),
            sentiment: sentiment.compound || 0,
            source: 'Reddit',
            timestamp,
